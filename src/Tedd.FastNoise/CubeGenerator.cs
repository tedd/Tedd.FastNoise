...
namespace Tedd.FastNoise
{
    public class Generator
    {
        ...
        public void Fill(in Span<double> span, in int sizeX, in int sizeY)
        {
            ...
            var x = Vector<double>.Zero;
            var y = Vector<double>.Zero;

            var z = Vector<double>.Zero;
            for (; Vector.LessThan(x, vsx) == Vector<long>.Zero; Vector.Add(x, Vector<double>.One))
            {
                for (; Vector.LessThan(y, vsy) == Vector<long>.Zero; Vector.Add(y, Vector<double>.One))
                {
                    var xi = Vector.AsVectorByte(x)[0];
                    var yi = Vector.AsVectorByte(y)[0];
                    var zi = Vector.AsVectorByte(z)[0];
                    var u = x;
                    var v = y;
                    var w = z;
                    var xf = Vector<double>.Zero;
                    var yf = Vector<double>.Zero;
                    var zf = Vector<double>.Zero;
