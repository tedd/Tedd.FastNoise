// Minimal headless Vulkan host for the sample. Device setup and readback belong to the application.
// A renderer should reuse its own device and device-local buffers; this host reads back only for inspection.
using System;
...
internal sealed unsafe class HeadlessVulkan : IDisposable
{
    ...
    // Buffer ranges can share an allocation; respect the storage-buffer alignment.
    private ulong _offset;
    private const ulong Capacity = 4 * 1024 * 1024;

    internal HeadlessVulkan()
    {
        try
        {
